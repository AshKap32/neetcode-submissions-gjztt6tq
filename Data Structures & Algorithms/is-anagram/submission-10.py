class Solution:
    def isAnagram(self, s: str, t: str) -> bool:
        ...
        for i in range(len(s)):
            # Ensures to increase the count by x amount 
            # for the character we are currently on for s and t respectively
            # We could instead calc the ord for both then just do += 1 but it gives the same answer
            count_s[s[i]] += 1
            count_t[t[i]] += 1
